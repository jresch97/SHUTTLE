...
static COS_CLASS gui_wdg_class = NULL;

static void gui_wdg_pr1nt(GUI_WIDGET this, int level)
{
        size_t i;
        ...
        printf("\n");
        while (curr) {
                gui_wdg_pr1nt(curr, level + 1);
                curr = GUI_WIDGET_NEXT(curr);
        }
}

static void gui_wdg_print_method(GUI_WIDGET this)
{
        printf("[%4d,%4d,%4d,%4d] ~",
...
{
        COS_CLASS_INFO info;
        if (gui_wdg_class) return gui_wdg_class;
        if (cos_class_lookup(GUI_WIDGET_CLASS_NAME,
                             &gui_wdg_class)) {
                return gui_wdg_class;
        }
        info.name        = GUI_WIDGET_CLASS_NAME;
        info.parent      = COS_OBJECT_TYPE;
        info.class.size  = sizeof(struct GUI_WIDGET_CLASS_S);
        info.class.ctor  = gui_wdg_class_ctor;
        info.class.dtor  = gui_wdg_class_dtor;
        info.inst.size   = sizeof(struct GUI_WIDGET_S);
        info.inst.ctor   = gui_wdg_ctor;
        info.inst.dtor   = gui_wdg_dtor;
        info.inst.params = cos_params(1, "layout", COS_TYPE_INT);
...
void gui_wdg_class_ctor(COS_CLASS class)
{
        ...
        GUI_WIDGET_CLASS_PRINT(class) = gui_wdg_print_method;
}
...
void gui_wdg_ctor(COS_OBJECT this, COS_VALUES values)
{
        ...
        GUI_WIDGET_LAYOUT(this) = cos_unbox_int(cos_values_at(values, 0));
}
...
{
        GUI_WIDGET curr;
        switch (GUI_WIDGET_LAYOUT(this)) {
                case GUI_LAYOUT_VBOX:
                        gui_layout_vbox(this);
                        break;
                case GUI_LAYOUT_HBOX:
                        gui_layout_hbox(this);
                        break;
        }
        curr = GUI_WIDGET_CHILD(this);
        while (curr) {
